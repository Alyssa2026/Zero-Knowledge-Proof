...
- [Video](https://drive.google.com/file/d/1TLWvnhGV7MxJhytQGfPi1CLTrKwLIjnM/view?usp=drive_link)

- [Demo slides](https://docs.google.com/presentation/d/1f05vo6Dh-0K5KE950Fz1mIfhSCe7LrZxXtftO_Vk6lE/edit#slide=id.gc6f90357f_0_13) 

- [Main academic source](https://people.csail.mit.edu/silvio/Selected%20Scientific%20Papers/Zero%20Knowledge/Proofs_That_Yield_Nothing_But_Their_Validity_or_All_Languages_in_NP_Have_Zero-Knowledge_Proof_Systems.pdf) 
...
Across our time working with this model, we stumbled upon several challenges and had to adapt our project to the requirements of Forge. For one, the original procedure is probabilistic — the “soundness” property is based on the overwhelming probability that a “cheating” prover gets caught. This means that some of the supposed properties of the system do not carry over to the model; the model is not deterministically sound, and we show that fact in some of our tests. m^2 rounds would also lead to prohibitively long trace lengths, so we ignored that constraint given that it only serves the probabilistic nature of the procedure that we omit anyways. The “random” permutation of colors is also not feasibly modeled by a satisfiability solver, but that’s ok since we fully explore the space of possible permutations instead. 

We also went into the project wondering if it would be possible to prove zero-knowledgeness as a property of our model but learned that such a task would involve higher-order quantification and mathematics outside of the domain of a simple boolean satisfiability solver. We alternatively discuss at least a rough sketch of what it would look like to broach such a question.
